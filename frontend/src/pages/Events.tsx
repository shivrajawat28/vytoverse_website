...
export default function Events() {
  ...
  const tabs: { key: TabType; label: string; count: number }[] = [
    ...
    { key: 'upcoming', label: 'Upcoming', count: events.filter((e) => e.status === 'upcoming' || e.status === 'ongoing').length },
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="text-lg text-vyto-text-secondary max-w-2xl mx-auto">
              From hackathons to workshops, there's always something exciting happening at VytoVerse.
            </p>
          </motion.div>
                ...
                <button
                  ...
                  className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                    activeTab === tab.key
                      ...
                      : 'text-vyto-text-muted hover:text-white'
                  }`}
                >
                  {tab.label} ({tab.count})
                </button>
          ...
          {loading ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="glass-card overflow-hidden animate-pulse">
                  ...
                  <div className="p-6">
                    <div className="h-4 bg-vyto-border rounded w-24 mb-4" />
                    <div className="h-6 bg-vyto-border rounded w-3/4 mb-3" />
                    <div className="h-4 bg-vyto-border rounded w-full mb-2" />
                    <div className="h-4 bg-vyto-border rounded w-2/3 mb-4" />
            ...
            <div className="text-center py-20">
              ...
              <p className="text-vyto-text-muted">Check back later for upcoming events.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  >
                    ...
                    {event.image ? (
                      <div className="h-44 overflow-hidden relative">
                        ...
                        <span className={`absolute top-3 left-3 px-2.5 py-1 rounded-md text-xs font-semibold ${sc.class}`}>
                          {sc.label}
                        </span>
                      </div>
                    ) : (
                    ...
                    <div className="p-6">
                      {/* Date & status for no-image cards */}
                      {!event.image && (
                        <div className="flex items-center justify-between mb-3">
                          <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${sc.class}`}>
                            {sc.label}
                          </span>
                        </div>
                      )}

                      <div className="flex items-center gap-1.5 text-xs text-vyto-text-muted mb-2">
                      ...
                      <div className="space-y-1.5 mb-4">
                        {event.time_start && (
                          <div className="flex items-center gap-2 text-xs text-vyto-text-muted">
                      ...
                      {event.registration_url && event.status === 'upcoming' && (
                        ...
                        >
                          Register <ExternalLink className="w-4 h-4 group-hover/btn:translate-x-0.5 transition-transform" />
                        </a>
                      )}
                    </div>
                  </motion.div>
